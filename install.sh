...
DB_NAME=$6
ODOO_VERSION="18.0"
# Service and system user name without dots
SERVICE_NAME="odoo18"
ODOO_USER="$SERVICE_NAME"
ODOO_HOME="/opt/odoo"
...
sudo chown -R $ODOO_USER:$ODOO_USER /var/log/$SERVICE_NAME /var/lib/$SERVICE_NAME
...
echo "#7 - Criando arquivo de configuração /etc/$SERVICE_NAME.conf..."
sudo tee /etc/$SERVICE_NAME.conf > /dev/null <<EOF
[options]
; Servidor
...
; Logs
debug_mode = False
logfile = /var/log/$SERVICE_NAME/odoo.log
EOF
sudo chown $ODOO_USER:$ODOO_USER /etc/$SERVICE_NAME.conf
sudo chmod 640 /etc/$SERVICE_NAME.conf

# 8. Configurar serviço systemd
echo "#8 - Criando service $SERVICE_NAME.service..."
...
User=$ODOO_USER
Group=$ODOO_USER
ExecStart=$ODOO_HOME/venv/bin/python3 $ODOO_HOME/$ODOO_VERSION/odoo-bin -c /etc/$SERVICE_NAME.conf
